...
import type { VolumeWithProgress } from "@db/schema";
import type { Volume } from "../model/types";

export function volumeRowToVolume(row: VolumeWithProgress): Volume {
  ...
  return {
    ...
    progress: row.progress?.lastPage != null
      ? row.progress.lastPage / row.totalPages
      : 0,
  };
}
